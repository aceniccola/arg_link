# our verifier agent must output some boolean value. Still unclear on how to force this.
...
# TODO: can you add a verifier as a tool that acts like the reflect node in the graph to our compiled graph. 
# If it can be added as a tool it may not need to be refactored into a graph. 
# However, I think this is unavoidable at this time.

# It would be best to find a way to scrape away the indecies of the headers. Perhaps we don't give the headings in at all. 
...
agent_graph = create_react_agent(model = high_thinking_model, checkpointer=MemorySaver()) 
